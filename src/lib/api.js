...
/** Step 1 of passkey signup: get registration options for a *new* account. */
export async function apiPasskeySignupOptions(email, displayName, avatar) {
  ...
  return data; // { options, challengeToken }
}

/** Step 2 of passkey signup: verify attestation, create account, get JWT. */
...
/** Step 1 of passkey login: get authentication options. */
...
/** Step 2 of passkey login: verify assertion, get JWT. */
...
/** Add a passkey to an already-authenticated account (step 1). */
export async function apiPasskeyRegisterOptions() {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || 'Failed to get register options');
  return data; // { options, challengeToken }
}

/** Add a passkey to an already-authenticated account (step 2). */
export async function apiPasskeyRegisterVerify(challengeToken, attResp) {
  const res = await fetch(`${BASE}/api/auth/passkey/register-verify`, {
    method: 'POST',
    headers: authHeaders(),
    body: JSON.stringify({ challengeToken, attResp }),
  });
